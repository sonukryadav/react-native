...
export default function LoginPage() {
    ...
    React.useEffect(() => {
        ...
        if (length >= 8 && upperCharLength && lowerCharLength && digitLength && specialCharLength) {
            setActive(false);
        } else {
            setActive(true)
        }
    ...
    return (
        <SafeAreaView style={styles.container}>
            <ScrollView>
                ...
                />
                <Text style={{ fontSize: 14, marginHorizontal: 20, marginVertical: -15, marginBottom: 20 }}>{emailV && "❌ Invalid Email"}</Text>
                <TextInput
                    style={styles.input1}
                ...
                <Pressable
                    ...
                    disabled={active}
                >
                    {({ pressed }) => (<Text style={{ ...styles.textPressable, backgroundColor: (!emailV && !active) ? "green" : "grey" }}>LOGIN</Text>)}
                </Pressable>
